...
def orthogonal_loss(x_pool):
    xps = x_pool.view(x_pool.size(0), x_pool.size(1), 1).permute(0, 2, 1)
    xp1, xp2, xp3 = torch.chunk(xps, 3, 0)
    xpss = torch.cat((xp2, xp3), 1)
    loss_ort = torch.triu(torch.bmm(xpss, xpss.permute(0, 2, 1)), diagonal=1).sum() / (x_pool.size(0))
    return loss_ort
...
class CPMLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        loss_123 = self.ranking_loss(dist_an_123, dist_ap_123, torch.ones_like(dist_an_123)) + \
                   (
                           self.ranking_loss(dist_an_33, dist_ap_123, torch.ones_like(dist_an_33)) +
                           self.ranking_loss(dist_an_11, dist_ap_123, torch.ones_like(dist_an_33))
                   ) * 0.5
        loss_124 = self.ranking_loss(dist_an_124, dist_ap_124, torch.ones_like(dist_an_124)) + \
                   (
                           self.ranking_loss(dist_an_44, dist_ap_124, torch.ones_like(dist_an_44)) +
                           self.ranking_loss(dist_an_22, dist_ap_124, torch.ones_like(dist_an_44))
                   ) * 0.5
...
class OriTripletLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        dist = torch.pow(inputs, 2).sum(dim=1, keepdim=True).expand(n, n).type(inputs.dtype)
        dist = dist + dist.t()
...
def pdist_torch(emb1, emb2):
    ...
    emb1_pow = torch.pow(emb1, 2).sum(dim = 1, keepdim = True).expand(m, n).type(emb1.dtype)
    emb2_pow = torch.pow(emb2, 2).sum(dim = 1, keepdim = True).expand(n, m).t().type(emb2.dtype)
